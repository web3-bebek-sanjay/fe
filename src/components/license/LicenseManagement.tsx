        ...
        <div className="border-b border-slate-200 dark:border-slate-700 px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-1.5 rounded ${viewMode === "grid" ? "bg-slate-100 dark:bg-slate-700" : ""}`}
            >
              <GridIcon size={18} className="text-slate-600 dark:text-slate-400" />
            </button>
            <button
              onClick={() => setViewMode("history")}
              className={`p-1.5 rounded ${viewMode === "history" ? "bg-slate-100 dark:bg-slate-700" : ""}`}
            >
              <ListIcon size={18} className="text-slate-600 dark:text-slate-400" />
            </button>
          </div>
        </div>
        <div className="p-4">{viewMode === "grid" ? <IPCardGrid searchQuery={searchQuery} /> : <LicenseHistory />}</div>
      </div>
    </div>
  )
}
