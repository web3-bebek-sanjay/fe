...
export const IPCardGrid: React.FC<IPCardGridProps> = ({ searchQuery }) => {
  ...
  const filteredIPs = useMemo(() => {
    return ipData.filter((ip) => {
      ...
      const category =
        typeof ip.category === 'string' ? ip.category : ip[3]?.toString() || '';
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-10">
        ...
        <p className="text-slate-500">Loading IP assets...</p>
      </div>
    );
  }
    ...
    <div>
      {filteredIPs.length === 0 ? (
        <div className="text-center py-8 text-slate-500">
          No IP assets found matching your search criteria.
        </div>
      ) : (
